...
Contract('Migrator', ([alice, bob, dev, minter]) => {
 ...
 it('should do the migration successfully', async () => {
  await this.token.transfer(this.lp1.address, '100000000', { from: minter });
  ...
  await this.token.transfer(this.lp1.address, '100000', { frm: minter });
 ...
 it('should allow first minting from public only after migration is done', async () => {
  await this.factory2.setMigrator(this.migrator.address, { from: alice });
  this.tokenx = await MockERC20.new('TOKENX', 'TOKENX', '100000000', { from: minter });
  this.lpx = await UniswapV2Pair.at((await this.factory2.createPair(
   this.weth.address, this.tokenx.address)).logs[0].args.pair);
  ...
  await expectRevert(this.lpx.mint(minter), 'Must not have a migrator');
